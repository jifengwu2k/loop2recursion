; ModuleID = './benchmark_intermediate_representations/loop2recursion/sha/sha_driver.ll.temp'
...
entry:
  ...
  %tmp = bitcast %struct.SHA_INFO* %sha_info to i8*
  call void @llvm.lifetime.start.p0i8(i64 184, i8* %tmp) #4
...
"main$0_initialize_context":                      ; preds = %while.body.preheader
  store %struct.SHA_INFO* %sha_info, %struct.SHA_INFO** @"./benchmark_intermediate_representations/baseline/sha/sha_driver.ll_invariant_variable$0"
...
if.then:                                          ; preds = %entry
  %tmp1 = load %struct._IO_FILE*, %struct._IO_FILE** @stdin, align 8, !tbaa !1
  call void @sha_stream(%struct.SHA_INFO* nonnull %sha_info, %struct._IO_FILE* %tmp1) #4
...
if.end6:                                          ; preds = %if.end6.loopexit, %if.then, %while.cond.preheader
  call void @llvm.lifetime.end.p0i8(i64 184, i8* %tmp) #4
  ret i32 0
}
...
"main$0_header":
  ...
  %tmp = load i8*, i8** %incdec.ptr, align 8, !tbaa !1
  %call = call %struct._IO_FILE* @fopen(i8* %tmp, i8* nonnull getelementptr inbounds ([3 x i8], [3 x i8]* @.str, i64 0, i64 0))
  %cmp1 = icmp eq %struct._IO_FILE* %call, null
  br i1 %cmp1, label %if.then2, label %if.else4

if.else4:                                         ; preds = %"main$0_header"
  %tmp2 = load %struct.SHA_INFO*, %struct.SHA_INFO** @"./benchmark_intermediate_representations/baseline/sha/sha_driver.ll_invariant_variable$0"
  call void @sha_stream(%struct.SHA_INFO* nonnull %tmp2, %struct._IO_FILE* nonnull %call) #4
  %tmp3 = load %struct.SHA_INFO*, %struct.SHA_INFO** @"./benchmark_intermediate_representations/baseline/sha/sha_driver.ll_invariant_variable$0"
  call void @sha_print(%struct.SHA_INFO* nonnull %tmp3) #4
  %call5 = call i32 @fclose(%struct._IO_FILE* nonnull %call)
  br label %"main$0_latch$0"

if.then2:                                         ; preds = %"main$0_header"
  %tmp4 = load i8*, i8** %incdec.ptr, align 8, !tbaa !1
  %call3 = call i32 (i8*, ...) @printf(i8* nonnull getelementptr inbounds ([30 x i8], [30 x i8]* @.str.1, i64 0, i64 0), i8* %tmp4)
  br label %"main$0_latch$0"
...
"main$0_recursively_call_function":               ; preds = %"main$0_increment_depth_counter"
  call void @"main$0"(i8** %incdec.ptr, i32 %dec)
  ret void
