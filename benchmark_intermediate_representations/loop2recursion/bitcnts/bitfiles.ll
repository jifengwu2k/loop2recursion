; ModuleID = './benchmark_intermediate_representations/loop2recursion/bitcnts/bitfiles.ll.temp'
...
entry:
  ...
  %tmp = bitcast i8* %call to %struct.bfile*
...
if.end5:                                          ; preds = %if.end
  %tmp1 = getelementptr inbounds i8, i8* %call, i64 9
  store i8 0, i8* %tmp1, align 1, !tbaa !6
  %tmp2 = getelementptr inbounds i8, i8* %call, i64 11
  store i8 0, i8* %tmp2, align 1, !tbaa !7
  br label %cleanup

cleanup:                                          ; preds = %if.end5, %if.then4, %entry
  %retval.0 = phi %struct.bfile* [ null, %if.then4 ], [ %tmp, %if.end5 ], [ null, %entry ]
...
entry:
  ...
  %tmp = load i8, i8* %rcnt, align 1, !tbaa !6
...
if.then:                                          ; preds = %entry
  ...
  %tmp1 = load %struct._IO_FILE*, %struct._IO_FILE** %file, align 8, !tbaa !1
  %call = tail call i32 @fgetc(%struct._IO_FILE* %tmp1)
...
if.end:                                           ; preds = %if.then, %entry.if.end_crit_edge
  %tmp2 = phi i8 [ %conv2, %if.then ], [ %.pre, %entry.if.end_crit_edge ]
  %tmp3 = phi i8 [ 7, %if.then ], [ %phitmp, %entry.if.end_crit_edge ]
  store i8 %tmp3, i8* %rcnt, align 1, !tbaa !6
  %conv6 = sext i8 %tmp2 to i32
  %conv8 = sext i8 %tmp3 to i32
  %shl = shl i32 1, %conv8
...
entry:
  ...
  %tmp = load i8, i8* %wcnt, align 1, !tbaa !7
  %cmp = icmp eq i8 %tmp, 8
...
if.then:                                          ; preds = %entry
  %tmp1 = load i8, i8* %wbuf, align 2, !tbaa !9
  %conv2 = sext i8 %tmp1 to i32
  %file = getelementptr inbounds %struct.bfile, %struct.bfile* %bf, i64 0, i32 0
  %tmp2 = load %struct._IO_FILE*, %struct._IO_FILE** %file, align 8, !tbaa !1
  %call = tail call i32 @fputc(i32 %conv2, %struct._IO_FILE* %tmp2)
  store i8 0, i8* %wcnt, align 1, !tbaa !7
  br label %if.end

if.end:                                           ; preds = %if.then, %entry.if.end_crit_edge
  %tmp3 = phi i8 [ %phitmp, %entry.if.end_crit_edge ], [ 1, %if.then ]
  store i8 %tmp3, i8* %wcnt, align 1, !tbaa !7
  %tmp4 = load i8, i8* %wbuf, align 2, !tbaa !9
  %shl = shl i8 %tmp4, 1
  %and = and i32 %bit, 1
...
entry:
  %file = getelementptr inbounds %struct.bfile, %struct.bfile* %bf, i64 0, i32 0
  %tmp = load %struct._IO_FILE*, %struct._IO_FILE** %file, align 8, !tbaa !1
  %call = tail call i32 @fclose(%struct._IO_FILE* %tmp)
  %tmp1 = bitcast %struct.bfile* %bf to i8*
  tail call void @free(i8* %tmp1) #2
  ret void
}
