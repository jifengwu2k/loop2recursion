...
void LoopConverter::create_auxiliary_basic_blocks_in_outer_function() {
    ...
    // create initialize_context if use_return_value_struct, recursion_depth_limit
    if (loop_information.use_return_value_struct || recursion_depth_limit) {
...
void LoopConverter::modify_preheader() {
    llvm::BasicBlock* new_successor = (loop_information.use_return_value_struct || recursion_depth_limit) ? initialize_context : call_recursive_function;
...
void LoopConverter::modify_initialize_context() {
    if (loop_information.use_return_value_struct || recursion_depth_limit) {
        // insert alloca_return_value_struct in initialize_context if use_return_value_struct
        if (loop_information.use_return_value_struct) {
            alloca_return_value_struct = new llvm::AllocaInst(
                loop_information.return_value_struct_type, // llvm::Type * Ty
                0, // unsigned AddrSpace, Address space 0 is the default address space
                "", // const Twine& Name
                initialize_context // BasicBlock* InsertAtEnd
            );
        }

        // insert alloca_recursion_state_struct in initialize_context if recursion_depth_limit
        if (recursion_depth_limit) {
            alloca_recursion_state_struct = new llvm::AllocaInst(
                loop_information.recursion_state_struct_type, // llvm::Type * Ty
                0, // unsigned AddrSpace, Address space 0 is the default address space
                "", // const Twine& Name
                initialize_context // BasicBlock* InsertAtEnd
            );
...
void LoopConverter::synthesize_parameter_list() {
    ...
    number_of_parameters = loop_information.use_return_value_struct ? 1 : 0;

    number_of_parameters += loop_information.phi_nodes_in_header.size();

    number_of_parameters += loop_information.captured_values.size();
    
    if (recursion_depth_limit) number_of_parameters += 2;

    // initialize types_in_parameter_list, index_of_return_value_struct_in_parameter_list, indices_of_loop_variables_and_captured_values_in_parameter_list, index_of_recursion_state_struct_in_parameter_list, index_of_recursion_depth_counter_in_parameter_list
    types_in_parameter_list.reserve(number_of_parameters);

    size_t current_index = 0;

    if (loop_information.use_return_value_struct) {
        types_in_parameter_list.push_back(loop_information.pointer_to_return_value_struct_type);

        index_of_return_value_struct_in_parameter_list = current_index++;
    }
    ...
    for (llvm::PHINode* loop_variable: loop_information.phi_nodes_in_header) {
        ...
        indices_of_loop_variables_and_captured_values_in_parameter_list[loop_variable] = current_index++;
    }

    for (llvm::Value* captured_value: loop_information.captured_values) {
        types_in_parameter_list.push_back(captured_value->getType());

        indices_of_loop_variables_and_captured_values_in_parameter_list[captured_value] = current_index++;
    }

    if (recursion_depth_limit) {
        types_in_parameter_list.push_back(loop_information.pointer_to_recursion_state_struct_type);

        index_of_recursion_state_struct_in_parameter_list = current_index++;
...
void LoopConverter::create_recursive_function() {
    ...
    // initialize return_value_struct_argument if use_return_value_struct
    if (loop_information.use_return_value_struct) {
        return_value_struct_argument = recursive_function->getArg(index_of_return_value_struct_in_parameter_list);
    }

    // initialize loop_variable_arguments
    for (llvm::PHINode* loop_variable: loop_information.phi_nodes_in_header) {
        size_t index = indices_of_loop_variables_and_captured_values_in_parameter_list[loop_variable];

        loop_variable_arguments[loop_variable] = recursive_function->getArg(index);
    }

    // initialize captured_value_arguments
    for (llvm::Value* captured_value: loop_information.captured_values) {
        size_t index = indices_of_loop_variables_and_captured_values_in_parameter_list[captured_value];

        captured_value_arguments[captured_value] = recursive_function->getArg(index);
    }

    // initialize recursion_state_struct_argument, recursion_depth_counter_argument if recursion_depth_limit
    if (recursion_depth_limit) {
        recursion_state_struct_argument = recursive_function->getArg(index_of_recursion_state_struct_in_parameter_list);
...
void LoopConverter::modify_recursively_call_function() {
    ...
    if (loop_information.use_return_value_struct) {
        recursively_call_function_arguments[index_of_return_value_struct_in_parameter_list] = return_value_struct_argument;
    }

    for (llvm::PHINode* phi_node: loop_information.phi_nodes_in_header) {
        size_t index = indices_of_loop_variables_and_captured_values_in_parameter_list[phi_node];
    ...
    for (llvm::Value* captured_value: loop_information.captured_values) {
        ...
        recursively_call_function_arguments[index] = captured_value_arguments[captured_value];
    }

    if (recursion_depth_limit) {
        recursively_call_function_arguments[index_of_recursion_state_struct_in_parameter_list] = recursion_state_struct_argument;
...
void LoopConverter::modify_save_recursion_state() {
    if (recursion_depth_limit) {
        // store value_in_next_iteration in recursion_state_struct_argument
        for (llvm::PHINode* phi_node: loop_information.phi_nodes_in_header) {
            llvm::Value* value_in_next_iteration = selected_values_of_loop_variables_in_next_iteration[phi_node];
            ...
            llvm::GetElementPtrInst* element_pointer = llvm::GetElementPtrInst::Create(
                ...
                recursion_state_struct_argument, // Value* Ptr
                {
                    llvm::ConstantInt::get(
        ...
        // store false in recursion_state_struct_argument
...
void LoopConverter::modify_return_from_recursive_function_blocks() {
    ...
    for (auto it = exit_blocks_and_return_from_recursive_function_blocks.begin(), end = exit_blocks_and_return_from_recursive_function_blocks.end(); it != end; ++it) {
        ...
        if (loop_information.use_return_value_struct) {
            ...
            // store values_to_return in return_value_struct_argument
            for (llvm::PHINode* phi_node_in_exit_block: loop_information.exit_blocks_and_phi_nodes[exit_block]) {
                ...
                llvm::GetElementPtrInst* return_value_struct_element_pointer = llvm::GetElementPtrInst::Create(
                    ...
                    return_value_struct_argument, // Value* Ptr
                    {
                        llvm::ConstantInt::get(
                            llvm::Type::getInt32Ty(loop_information.context), // IntegerType* Ty
                            0, // uint64_t V
            ...
            // store exit_block_id in return_value_struct_argument
...
void LoopConverter::modify_call_recursive_function_and_extract_next_values() {
    ...
    if (recursion_depth_limit) {
        for (llvm::PHINode* loop_variable: loop_information.phi_nodes_in_header) {
            llvm::GetElementPtrInst* get_loop_variable_ptr = llvm::GetElementPtrInst::Create(
                loop_information.recursion_state_struct_type, // Type* PointeeType
                alloca_recursion_state_struct, // Value* Ptr
                {
                    llvm::ConstantInt::get(
                        llvm::Type::getInt32Ty(loop_information.context), // IntegerType* Ty
                        0, // uint64_t V
    ...
    if (loop_information.use_return_value_struct) {
        arguments[index_of_return_value_struct_in_parameter_list] = alloca_return_value_struct;
    }
    ...
    for (llvm::Value* captured_value: loop_information.captured_values) {
        size_t index = indices_of_loop_variables_and_captured_values_in_parameter_list[captured_value];
        arguments[index] = captured_value;
    }

    if (recursion_depth_limit) {
        arguments[index_of_recursion_state_struct_in_parameter_list] = alloca_recursion_state_struct;
...
void LoopConverter::modify_branch_to_exit() {
    if (loop_information.exit_blocks.size() > 1) {
        // load_exit_block_id from alloca_return_value_struct
        llvm::GetElementPtrInst* get_exit_block_id_ptr = llvm::GetElementPtrInst::Create(
            loop_information.return_value_struct_type, // Type* PointeeType
            alloca_return_value_struct, // Value* Ptr
            {
                llvm::ConstantInt::get(
                    llvm::Type::getInt32Ty(loop_information.context), // IntegerType* Ty
                    0, // uint64_t V
...
void LoopConverter::replace_captured_values() {
    ...
    for (llvm::Value* captured_value: loop_information.captured_values) {
        captured_value->replaceUsesWithIf(
            captured_value_arguments[captured_value], // Value* New
            [this](llvm::Use& use) {
                ...
                if (llvm::Instruction* instruction = llvm::dyn_cast<llvm::Instruction>(user)) {
                    ...
                    return function == recursive_function;
                }
                else {
                    return false;
                }
